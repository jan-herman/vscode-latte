...
export class LatteTagsProcessor {
	...
	public async scan(doc: TextDocument, latteFileInfo: LatteFileInfo): VoidPromise {
		...
		for (const tag of parsed) {
			...
			if (isInstanceOf(tag, ParametersTag)) {
				narrowType<ParametersTag>(tag)
				await this.processParametersTag(latteFileInfo.variables, tag, doc)
			}
	...
	private async processParametersTag(
		...
		for (const parameter of tag.parameters) {
			const varInfo: VariableInfo = {
				...
				exprType: null,
				definedAt: await getPositionAtOffset(parameter.nameRange.startOffset, doc),
			}
