...
export function injectPoisIntoDumbTag(tag: AbstractTag): void {
	...
	switch (true) {
		case isInstanceOf(tag, VarTypeTag):
			...
			break
		case isInstanceOf(tag, ParametersTag):
...
function buildParametersTagPois(tag: ParametersTag): AbstractPoi[] {
	return tag.parameters.flatMap((parameter) => {
		return [
			{
				type: PoiType.Hover,
				range: [parameter.typeRange.startOffset, parameter.typeRange.endOffset],
				contentFn: async (
					doc: TextDoc,
					position: vscode.Position,
					extCore: ExtensionCore,
				) => {
					const repr = getPhpTypeRepr(parameter.varType)
					return `_type_ \`${repr}\``
				},
			} as HoverPoi,
			{
				...
				contentFn: async (
					...
					const locationLink: vscode.LocationLink = {
						...
						targetRange: ZeroVoidRange,
						originSelectionRange: new vscode.Range(
							await getPositionAtOffset(parameter.typeRange.startOffset, doc),
							await getPositionAtOffset(parameter.typeRange.endOffset, doc),
						),
					}
